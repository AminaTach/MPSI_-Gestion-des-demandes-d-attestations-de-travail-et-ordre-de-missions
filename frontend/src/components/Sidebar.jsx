...
const Sidebar = ({ role, setRole }) => {
  ...
  const menuItems = {
    ...
    rh: [
      ...
      { label: "Ordre de mission", icon: ordre, path: "/rh/ordremissiontable" },
  ...
  const getItemClass = (path) => {
    ...
    return `px-2 py-2 rounded-lg flex flex-row gap-2 cursor-pointer ${
      isActive
        ? "bg-bleu_selected"
        : "hover:bg-bleu_selected hover:bg-opacity-50"
    }`;
  };
    ...
    return (
      <div
        className={`flex flex-col ${
          isMobile ? "text-violet" : "text-white"
        } space-y-2 text-lg text-center sm:text-start font-bold sm:text-sm  font-nunito`}
      >
        {itemsToRender.map((item) => (
          ...
          >
            {!isMobile && <img src={item.icon} alt="" />}
            {item.label}
          </li>
        ))}
      </div>
